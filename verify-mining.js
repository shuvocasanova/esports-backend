...
async function verifyMining() {
    try {
        ...
        await prisma.miningSubscription.deleteMany({});
        console.log('Cleared previous subscriptions.');

        // 1. Fetch Packages
        const packages = await prisma.miningPackage.findMany({ where: { status: 1 } });
        console.log(`Fetched active packages: ${packages.map(p => p.name).join(', ')}`);
        const starterPkg = packages.find(p => p.name === '1 days ');
        if (!starterPkg) {
            throw new Error('Starter ("1 days ") package not found in DB. Make sure you seeded.');
        }
        ...
        const qty = 3; // Lease 3 machines
        const subscribeAmount = parseFloat(starterPkg.rent_amount) * qty; // 1000 * 3 = 3000 USDT
        
        const startDate = new Date();
        const endDate = new Date(startDate.getTime() + starterPkg.duration_days * 24 * 60 * 60 * 1000);
        const rate = starterPkg.daily_rate; // '1.0000'
        ...
        const [updatedWallet, updatedUser, subscription] = await prisma.$transaction([
            ...
            prisma.miningSubscription.create({
                data: {
                    ...
                    quantity: qty,
                    rent_amount: subscribeAmount.toFixed(7),
                    daily_rate: rate,
        ...
        console.log(`Subscribed to "${starterPkg.name}" with quantity ${qty} (Total: ${subscribeAmount} USDT).`);
        console.log(`Post-subscription balances - User: ${updatedUser.balance}, Wallet: ${updatedWallet.coin_amount}`);
        if (parseFloat(updatedUser.balance) !== 7000.0 || parseFloat(updatedWallet.coin_amount) !== 7000.0) {
        ...
        await prisma.miningSubscription.update({
        ...
        await settleMiningPayouts(prisma);

        // Fetch updated subscription and balances
        const subAfterPayout = await prisma.miningSubscription.findUnique({
            where: { id: subscription.id }
        });
        ...
        const expectedInterest = subscribeAmount * (parseFloat(rate) / 100); // 3000 * 1% = 30 USDT
        ...
        if (parseFloat(userAfterPayout.balance) !== 7030.0) {
        ...
        const subCompleted = await prisma.miningSubscription.findUnique({
            where: { id: subscription.id }
        });
        ...
        const totalExpectedBalance = 7030.0 + secondPayoutInterest + subscribeAmount; // 7030 + 0 + 3000 = 10030
        ...
        const sub2 = await prisma.miningSubscription.create({
            data: {
                user_id: userId,
                package_id: starterPkg.id,
                quantity: 2,
                rent_amount: '2000.0000000',
                daily_rate: rate,
                total_earned: '0.0000000',
                status: 'active',
                start_date: new Date(),
        ...
        const balanceBeforeCancel = parseFloat(userCompleted.balance); // 10030
        // Deduct balance manually to simulate creation
        await prisma.user.update({
            where: { id: userId },
            data: { balance: (balanceBeforeCancel - 2000).toFixed(7) }
        });
        await prisma.wallet.update({
            where: { id: wallet.id },
            data: { coin_amount: (balanceBeforeCancel - 2000).toFixed(7) }
        });

        // Simulate cancel endpoint call (refund logic)
        const refundAmount = parseFloat(sub2.rent_amount);
        await prisma.$transaction([
            prisma.wallet.update({
                where: { id: wallet.id },
                data: { coin_amount: (balanceBeforeCancel - 2000 + refundAmount).toFixed(7) }
            }),
            prisma.user.update({
                where: { id: userId },
                data: { balance: (balanceBeforeCancel - 2000 + refundAmount).toFixed(7) }
            }),
        ...
        const subCancelled = await prisma.miningSubscription.findUnique({ where: { id: sub2.id } });
        ...
        console.log('\n=== ALL MINING VERIFICATION CHECKS PASSED SUCCESSFULLY ===');
